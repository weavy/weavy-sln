...
wvy.messenger = (function ($) {
        ...
        if (window.Notification) {
            ...
            if (Notification.permission === "granted" && wvy.context.notify) {
                var notification = new Notification("Message from " + (message.createdBy.name || message.createdBy.username), {
                    ...
                    icon: message.createdBy.thumbUrl.replace("{options}", "96x96-crop,both").replace(".svg", ".png")
                });
    ...
    $(function () {
        ...
        function onMessageReceived(e) {
            if (e.data.name === "open-conversation") {
                if (e.data.id) {
                    // set active conversation
                    set(e.data.id);
                } else {
        ...
        $textarea = $(".message-form textarea").removeAttr("disabled").emojioneArea({
            attributes: {
                dir: "ltr",
                spellcheck: true,
                autocomplete: "on",
