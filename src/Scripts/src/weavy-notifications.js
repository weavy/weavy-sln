...
(function ($) {
    ...
    $(document).on("click", "[data-entity-like]", function (e) {
        ...
        wvy.api.read(_notificationId).then(function () {
            wvy.api.like(type, id).then(function () {
                close();
            });
        });
    });
    ...
    $(document).on("click", "[data-entity-unlike]", function (e) {
        ...
        // REVIEW: show spinner during ajax call?
        wvy.api.read(_notificationId).then(function () {
            wvy.api.unlike(type, id).then(function () {
                close();
            });
        });
    });
    ...
    $(function() {
        ...
        if (comments) {
            try {
                _observer = new MutationObserver(reportHeight);
                _observer.observe(comments, { attributes: true, childList: true, characterData: false, subtree: true });
            } catch (e) { }
        }
    ...
    // insert comment
    var insertComment = function (e, indata, editor) {
        e.preventDefault();
        var $editor = $(editor);
